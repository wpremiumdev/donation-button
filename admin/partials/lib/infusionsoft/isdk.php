...
class iSDK {
     ...
     */
    public function cfgCon($name, $key = "", $app_name = "", $dbOn = "on", $type = "i") {
        ...
        $connInfo = array("{$name}:{$app_name}:i:{$key}:{$app_name}.infusionsoft.com");

        $appLines = $connInfo;
        foreach ($appLines as $appLine) {
            $details[substr($appLine, 0, strpos($appLine, ":"))] = explode(":", $appLine);
        }
        $appname = $details[$name][0];
        $type = $details[$name][2];
        $this->key = $details[$name][3];
     ...
     * @return bool
     */
    public function attachEmail($cId, $fromName, $fromAddress, $toAddress, $ccAddresses, $bccAddresses, $contentType, $subject, $htmlBody, $txtBody, $header, $strRecvdDate, $strSentDate, $emailSentType = 1) {
        $carray = array(
            php_xmlrpc_encode((int) $cId),
    ...
    public function infuDate($dateStr, $dateFrmt = 'US') {
        ...
        if ($dArray['error_count'] < 1) {
            $tStamp =
                    mktime($dArray['hour'], $dArray['minute'], $dArray['second'], $dArray['month'], $dArray['day'], $dArray['year']);
            if ($dateFrmt == 'UK') {
